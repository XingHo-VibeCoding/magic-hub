...
> 读图顺序：① 三个入口 → ② 内部两层 → ③ 三个出口；实线 = 阶段一数据流，虚线 = 阶段二数据流。

```mermaid
flowchart TB
    subgraph SRC["① 数据从哪来"]
        OFF["官方比赛资料<br/>真题 · 评分标准 · 教程<br/>整理为 JSON，随代码发布"]
        IN["用户粘贴的赛题文字<br/>一段文本，不落库"]
        UP["用户上传的作品 / skill<br/>论文 · 代码 · 图纸（阶段二）"]
    end

    subgraph SYS["② 经过谁（系统内部两层）"]
        FE["前端页面（跑在用户浏览器里）<br/>双区入口 · 资料库页 · 导航页 · AI 分析页<br/>纪律：不存密钥，不直连数据库"]
        CF1["云函数 ai-analyze<br/>校验：非空且不超过 5000 字<br/>注入 AI_API_KEY 并转发<br/>失败时返回错误码 TIMEOUT / UPSTREAM_ERROR"]
        CF2["云函数 works / skills<br/>校验后写入（阶段二）"]
    end

    subgraph DST["③ 到哪去"]
        SCR["用户屏幕<br/>题目概述 · 解题方向建议 · 坑点提示"]
        AI["AI 服务商<br/>收得到请求，收不到密钥"]
        DB[("CloudBase PostgreSQL<br/>阶段二启用<br/>users · works · skills 三表")]
    end

    OFF -->|"静态资源加载，不走后端"| FE
    IN -->|"粘贴"| FE
    FE -->|"POST /api/analyze<br/>只带请求，不带密钥"| CF1
    CF1 -->|"请求 + 密钥，只在云上这一段"| AI
    AI -->|"返回分析文本"| CF1
    CF1 -->|"组装好的三部分结果"| FE
    FE -->|"渲染显示"| SCR
    UP -.->|"阶段二：上传"| CF2
...
└─────────────────────────────────────────────────────┘      坑点提示）
      ...
      ▼
┌─────────────────────────────────────────────────────┐
│ 云函数 ai-analyze（后端，唯一带密钥的层）              │ ──► AI 服务商
│   · 校验：非空，且不超过 5000 字                      │    （收得到请求，
│   · 注入 AI_API_KEY 后转发                           │      收不到密钥）
│   · 组装三部分结果返回前端                             │
│   · 失败 → 错误码 TIMEOUT / UPSTREAM_ERROR           │
└─────────────────────────────────────────────────────┘
      │  结果三部分：题目概述 / 解题方向建议 / 坑点提示
      ▼
   前端页面渲染 ──► 用户看到结果

══════ 阶段二（数据流为虚线）══════

用户上传的作品 / skill ╌╌► 云函数 works/skills ╌╌► CloudBase PostgreSQL
（论文 / 代码 / 图纸）      （校验后写入 / 读取）     （阶段二启用，
                                                   只经后端读写）

图例：──►  阶段一数据流        ╌╌►  阶段二数据流
密钥走向：AI_API_KEY 只存在于云函数环境变量，浏览器端全程不可见（对应 PRD AC10）
```

### 5.3 数据从哪来、到哪去（一句话版）

- **来**：官方比赛资料由**人工整理成 JSON、随代码发布**（不走后端）；赛题文字来自**用户手动粘贴**；阶段二起，用户上传的论文／代码／图纸也来自用户。
- **经**：需要密钥或写库的数据全部经过**云函数**这道门——密钥在这里、校验也在这里；静态资料是唯一例外，前端直读。
- **去**：分析结果**直接回给当前页面**（阶段一不落库）；用户上传的内容**落进 PostgreSQL**；官方资料最终呈现在资料库页面上。
...
| 2026-09-20 | 按用户反馈重画 5.1/5.2 数据流图：补官方资料入口、错误路径（错误码）、密钥走向说明与阶段分界图例；5.3 一句话版同步补齐官方资料来路 |
